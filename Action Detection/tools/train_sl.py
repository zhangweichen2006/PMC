...
def single_test(model, data_loader, show=False):
    ...
    results = []
    pseudo_data_dict = {}
    pseudo_data_dict_rgb = {}
    pseudo_data_dict_flow = {}
...
def parse_args():
    ...
    parser.add_argument('--use_double_desc', type=bool, default=True)
...
def draw_graph(cfg, epoch_point, point_list, fuse_map_point):
    ...
    ver = os.getcwd().split('/')[-1]
    
    save_path = os.path.join('../', 'GRAPHS/', ver)
    if not os.path.exists(os.path.join('../', 'GRAPHS/')):
        os.makedirs(os.path.join('../', 'GRAPHS/'))
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    plt.savefig(os.path.join(save_path, '0.map.png'), bbox_inches='tight')
...
def main():
    ...
    for e in range(cfg.real_total_epochs):
        ...
        rgb_acc, flow_acc = train_detector(
            model,
            train_dataset,
            cfg,
        ...
        if e >= 0.2 * cfg.real_total_epochs: 
            ...
            print('---------------   Train Pseudo  --------------------')
            print('----------------------------------------------------')
            current_test_source_acc  = rgb_acc + flow_acc
            avg_test_source_acc = (sum_rgb_acc + sum_flow_acc)/(2*e)

            if args.use_double_desc:
            ...
            self_paced = 1 * sp_r / cfg.real_total_epochs

            print("self_paced ratio:", self_paced)
