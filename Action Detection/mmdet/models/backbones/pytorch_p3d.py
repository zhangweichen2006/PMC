...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,n_s=0,depth_3d=47,ST_struc=('A','B','C')):
        ...
        if self.id<self.depth_3d:
            ...
            self.bn2 = nn.BatchNorm3d(planes, track_running_stats=True)
            #
            self.conv3 = conv_T(planes,planes, stride=1,padding=(1,0,0))
            self.bn3 = nn.BatchNorm3d(planes, track_running_stats=True)
        else:
            self.conv_normal = nn.Conv2d(planes, planes, kernel_size=3, stride=1,padding=1,bias=False)
            self.bn_normal = nn.BatchNorm2d(planes, track_running_stats=True)

        if n_s<self.depth_3d:
            self.conv4 = nn.Conv3d(planes, planes * 4, kernel_size=1, bias=False)
            self.bn4 = nn.BatchNorm3d(planes * 4, track_running_stats=True)
        else:
            self.conv4 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
            self.bn4 = nn.BatchNorm2d(planes * 4, track_running_stats=True)
...
class P3D(nn.Module):
    ...
    def __init__(self, block, layers, modality='RGB',
        ...
        self.bn1 = nn.BatchNorm3d(64, track_running_stats=True) # bn1 is followed by conv1
        self.cnt=0
        self.relu = nn.ReLU(inplace=True)
            ...
            if stride != 1 or self.inplanes != planes * block.expansion:
                if shortcut_type == 'A':
                    ...
                    downsample = nn.Sequential(
                        ...
                        nn.BatchNorm3d(planes * block.expansion, track_running_stats=True)
                    )
    ...
    def forward(self, x):
        ...
        sizes=out3.size()
        # x = out3.view(-1,sizes[1],sizes[3],sizes[4])  #  Part Res5
        x = self.layer4(out3)
        out4 = x
        x = self.avgpool(x)
...
class P3D_two_stream(nn.Module):
    def __init__(self):
        super(P3D_two_stream, self).__init__()
        self.P3D_rgb = P3D199(pretrained=True, modality='RGB')
        self.P3D_flow = P3D199(pretrained=True, modality='Flow')

    # def init_weight(self, pretrain=True):
    #     if pretrain:
    #         i3d_weights = torch.load('rgb_imagenet.pt')
    #         i3d_base_weights = self.I3D_rgb.state_dict()
    #         # print(i3d_base_weights.keys())
    #         for k, v in i3d_base_weights.items():
    #             i3d_base_weights[k] = i3d_weights[k]
    #         self.I3D_rgb.load_state_dict(i3d_base_weights)
    #         i3d_flow_weights = torch.load('flow_imagenet.pt')
    #         i3d_flow_base_weights = self.I3D_flow.state_dict()
    #         for k, v in i3d_flow_base_weights.items():
    #             i3d_flow_base_weights[k] = i3d_flow_weights[k]
    #         self.I3D_flow.load_state_dict(i3d_flow_base_weights)
    ...
    def forward(self, x):
        # print(x.size())
        out_rgb = self.P3D_rgb(x[:,:20,:,:,:].permute(0,2,1,3,4))
        out_flow = self.P3D_flow(x[:,20:,:2,:,:].permute(0,2,1,3,4))
        return (out_rgb, out_flow)

    def train(self, mode=True):
        super(P3D_two_stream, self).train(mode)
        for m in self.modules():
        ...
        for l in [self.P3D_rgb.conv1_custom, self.P3D_rgb.layer1, self.P3D_rgb.layer2, self.P3D_rgb.layer3, self.P3D_flow.conv1_custom, self.P3D_flow.layer1, self.P3D_flow.layer2, self.P3D_flow.layer3]:
            for m in l.modules():
                for params in m.parameters():
                    params.requires_grad = False
