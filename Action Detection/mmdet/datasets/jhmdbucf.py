...
class JHMDBUCFDataset(Dataset):
	...
	def __init__(self,
		...
		self.img_ids, self.img_infos, self.hmdb_len, self.ucf_len = self._filter_imgs(test_mode=test_mode, small=small, trim=trim, pseudo_set=pseudo_set)

		self.set1_len = self.hmdb_len
		self.set2_len = self.ucf_len
		# else:
	...
	def _filter_imgs(self, min_size=32, test_mode=False, small=False, trim=False, pseudo_set=None):
		...
		if pseudo_set is None:
			...
			for key, pseu_value in pseudo_set.items():
				...
				det_boxes_rgb = all_boxes_rgb[0]
				det_boxes_flow = all_boxes_flow[0]
		...
		ucf_len = len(valid_ids) - hmdb_len
		print("ucf_len",ucf_len)

		
		return valid_ids, img_infos, hmdb_len, ucf_len
	...
	def __getitem__(self, idx):
		...
		while True:
			...
			data['datasetlen'] = DC(min(self.hmdb_len, self.ucf_len), cpu_only=True)
			...
			# data['pseudo_scores'] = DC(to_tensor(pseudo_bboxes_scores))
			# data['rev_ep'] = DC(self.revep, cpu_only=True)
