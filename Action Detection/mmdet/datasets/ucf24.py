...
class UCF24Dataset(Dataset):
	...
	def __init__(self,
		...
		self.img_ids, self.img_infos, self.ucf_len, self.ucf_len2 = self._filter_imgs(test_mode=test_mode, small=small, trim=trim, pseudo_set=pseudo_set)
		self.set1_len = self.ucf_len
		self.set2_len = self.ucf_len2
		# else:
	...
	def _filter_imgs(self, min_size=32, test_mode=False, small=False, trim=False, pseudo_set=None):
		...
		if test_mode and not self.pseudo_mode:
			if small:
		...
		if pseudo_set is None:
			...
			if not self.class_balanced:
				...
				ucf_len2 = len(valid_ids) - ucf_len
				print("ucf_len",ucf_len)
		...
		return valid_ids, img_infos, ucf_len, ucf_len2
	...
	def _parse_ann_info(self, ann_info, with_mask=True):
		...
		ann = dict(
			bboxes=gt_bboxes, labels=gt_labels, bboxes_ignore=gt_bboxes_ignore, pseudo_scores=pseudo_scores)
	...
	def __getitem__(self, idx):
		...
		while True:
			...
			for k, path in enumerate(image_paths):
				info = path.split('/')
				path_x = '/home/wzha8158/datasets/Action_DA/ucf24/flownet2-images/' + info[-3] + '/' + info[-2] + '/flow_x_' + info[-1]
				path_y = '/home/wzha8158/datasets/Action_DA/ucf24/flownet2-images/' + info[-3] + '/' + info[-2] + '/flow_y_' + info[-1]
