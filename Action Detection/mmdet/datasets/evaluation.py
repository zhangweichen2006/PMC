...
def ucf24_evaluate_detections(gt_boxes, det_boxes, CLASSES=[], iou_thresh=0.5):

    ap_strs = []
    num_frames = len(det_boxes)
    ...
    for cls_ind, cls in enumerate(CLASSES): # loop over each class 'cls'
        ...
        for nf in range(num_frames): # loop over each frame 'nf'
                ...
                frame_det_boxes = np.copy(det_boxes[nf][cls_ind]) # get frame detections for class cls in nf
