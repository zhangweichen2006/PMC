...
class DISCRIMINATIVE_DANN(nn.Module):
    def __init__(self, pre_trained, form_w, last_w, classes):
        ...
        self.disc_activate = Contrast_ReLU_activate(INI_DISC_SIG_SCALE, INI_DISC_A)
        ...
        self.domain_pred = nn.Sequential(nn.Linear(Bottleneck, 3072), nn.ReLU(True), nn.Dropout(),
                                         nn.Linear(3072, 2048), nn.ReLU(True), nn.Dropout(),
                                         nn.Linear(2048, 1))
        self.domain_pred_l1 = nn.Sequential(nn.Linear(Bottleneck, 3072), nn.ReLU(True), nn.Dropout(),
                                         nn.Linear(3072, 2048), nn.ReLU(True), nn.Dropout(),
                                         nn.Linear(2048, 1))
        self.domain_pred_l2 = nn.Sequential(nn.Linear(Bottleneck, 3072), nn.ReLU(True), nn.Dropout(),
                                         nn.Linear(3072, 2048), nn.ReLU(True), nn.Dropout(),
                                         nn.Linear(2048, 1))
        self.domain_pred_l3 = nn.Sequential(nn.Linear(Bottleneck, 3072), nn.ReLU(True), nn.Dropout(),
                                         nn.Linear(3072, 2048), nn.ReLU(True), nn.Dropout(),
                                         nn.Linear(2048, 1))
        ...
        self.source_bottleneck = nn.Sequential(nn.Linear(pre_trained.fc.in_features, Bottleneck))

        self.l1_bottleneck = nn.Sequential(nn.Linear(64, Bottleneck))
        self.l2_bottleneck = nn.Sequential(nn.Linear(128, Bottleneck))
        self.l3_bottleneck = nn.Sequential(nn.Linear(256, Bottleneck))

        self.source_classifier = nn.Sequential(nn.Linear(Bottleneck, self.num_class))
    ...
    def forward(self, cond, x1, x2=None, l=None,
        ...
        if (cond == 'pseudo_discriminator'):
            ...
            # disc_w, dom_w, conf, dom_sigma, dom_a = self.disc_activate(class_pred, dom_pred)
            # conf = self.disc_activate(class_pred, dom_pred)

            return class_pred, dom_pred.squeeze()
            # , conf
            # ,disc_w, dom_w, conf, dom_sigma, dom_a
