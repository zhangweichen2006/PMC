...
def iterator_reset(iterator_name_i, dset_name_i, dsets, batchsize_i, shuffle_i=True, drop_last_i=True, workers=0):
    dset_loaders = {}
    if batchsize_i > 0:
        if args.useRatio:
            if workers == 0:
            ...
            if workers == 0:
                dset_loaders[iterator_name_i] = torch.utils.data.DataLoader(dsets[dset_name_i],
                                                batch_size=batchsize_i, shuffle=shuffle_i, drop_last=drop_last_i)
            else:
                dset_loaders[iterator_name_i] = torch.utils.data.DataLoader(dsets[dset_name_i],
                                                batch_size=batchsize_i, shuffle=shuffle_i, drop_last=drop_last_i)
...
def iterator_update(iterator_i, dset_name_i, dsets, batchsize_i, pointer_i, total_pointer_i,  i_type="pseu", workers=0):
    if args.useRatio:
        if pointer_i + batchsize_i > len(iterator_i):
            iterator_i = iterator_reset(iterator_i, dset_name_i, dsets, batchsize_i, workers=workers)
            pointer_i = 0
...
def train(dsets, transforms, dset_loaders, train_source_iter, train_target_iter, train_source_iter2, train_target_iter2, model, model2,
    ...
    ax.annotate('Main, Main2: %0.4f %0.4f' % \
                (-0.1, -0.3), xy=(1.05, 0.4), xycoords='axes fraction')
