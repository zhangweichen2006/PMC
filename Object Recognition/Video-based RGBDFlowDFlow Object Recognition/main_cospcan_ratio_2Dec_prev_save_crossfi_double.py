...
def train(dsets, transforms, dset_loaders, train_source_iter, train_target_iter, train_source_iter2, train_target_iter2, model, model2,
    ...
    if epoch >= pre_epochs and epoch % args.skip == 0:
        ...
        Sorted_confid_set_co = sorted(Pseudo_set2, key=lambda tup: tup[2], reverse=True)
        ...
        s1 = sorted(Select_T1_set, key=lambda tup: tup[0])
        for s in range(len(s1)):
            if s < len(s1) - 1:
                if s1[0] == s1[s+1][0]:
                    if s1[s][1] == s1[s+1][1]:
                        # change 2,3,4
                        s1[s] = (s1[s][0], s1[s][1], (s1[s][2] + s1[s+1][2]) / 2, (s1[s][3] + s1[s+1][3]) / 2, (s1[s][4] + s1[s+1][4]) / 2, s1[s][5], s1[s][6])
                    else:
                        # different predicted c labels
                        if s1[s][2] < s1[s+1][2]:
                            s1[s] = s1[s+1]
        ...
        s12 = sorted(Select_T1T2_set, key=lambda tup: tup[0])

        for s in range(len(s12)):
            if s < len(s12) - 1:
                if s12[s][0] == s12[s+1][0]:
                    s12[s] = (s12[s][0], s12[s][1], (s12[s][2] + s12[s+1][2]) / 2, (s12[s][3] + s12[s+1][3]) / 2, (s12[s][4] + s12[s+1][4]) / 2, s12[s][5], s12[s][6])
                            
                    s12.remove(s12[s+1])

        Select_T1_set = s1
        # Select_T2_set = Sorted_Select_T2_set
        Select_T1T2_set = s12

        Select_T1_set2 = Select_T1_set
        # Select_T2_set2 = Select_T2_set
        Select_T1T2_set2 = Select_T1T2_set2
        ...
        if len(Select_T1_set2) > 0:
            dsets['pseudo'] = PathFolder(Select_T1_set2, num_segments=args.num_segments,
                                 new_length=data_length,
                                 modality=args.modality,
                                 image_tmpl="img_{:05d}.jpg" if args.modality in ["RGB", "RGBDiff"] else args.flow_prefix+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
        if len(Select_T2_set2) > 0:
            dsets['d_pseudo_target'] = PathFolder(Select_T2_set2, num_segments=args.num_segments,
                                 new_length=data_length,
                                 modality=args.modality,
                                 image_tmpl="img_{:05d}.jpg" if args.modality in ["RGB", "RGBDiff"] else args.flow_prefix+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
        ...
        if len(Select_T2_set2) > 0:
            dsets['d_pseudo_target_feat'] = PathFolder(Select_T2_set2, num_segments=args.num_segments,
                                 new_length=data_length,
                                 modality=args.modality,
                                 image_tmpl="img_{:05d}.jpg" if args.modality in ["RGB", "RGBDiff"] else args.flow_prefix+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
        ...
        if len(Select_T1T2_set2) > 0:
            if args.useT1Only:
                dsets['d_pseudo_all'] = PathFolder(Select_T1_set2, num_segments=args.num_segments,
                                 new_length=data_length,
                                 modality=args.modality,
                                 image_tmpl="img_{:05d}.jpg" if args.modality in ["RGB", "RGBDiff"] else args.flow_prefix+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
                dsets['d_pseudo_all_feat'] = PathFolder(Select_T1_set2, num_segments=args.num_segments,
                                     new_length=data_length,
                                     modality=args.modality,
                                     image_tmpl="img_{:05d}.jpg" if args.modality in ["RGB", "RGBDiff"] else args.flow_prefix+"{}_{:05d}.jpg",
                                     transform=transforms['target'])
            else:
                dsets['d_pseudo_all'] = PathFolder(Select_T1T2_set2, num_segments=args.num_segments,
                                 new_length=data_length,
                                 modality=args.modality,
                                 image_tmpl="img_{:05d}.jpg" if args.modality in ["RGB", "RGBDiff"] else args.flow_prefix+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
                dsets['d_pseudo_all_feat'] = PathFolder(Select_T1T2_set2, num_segments=args.num_segments,
                                 new_length=data_length,
                                 modality=args.modality,
                                 image_tmpl="img_{:05d}.jpg" if args.modality in ["RGB", "RGBDiff"] else args.flow_prefix+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
        ...
        if len(Select_T1_set) > 0:
            dsets['pseudo2'] = PathFolder(Select_T1_set, num_segments=args.num_segments,
                                 new_length=data_length2,
                                 modality=args.modality2,
                                 image_tmpl="img_{:05d}.jpg" if args.modality2 in ["RGB", "RGBDiff"] else args.flow_prefix2+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
        if len(Select_T2_set) > 0:
            dsets['d_pseudo_target2'] = PathFolder(Select_T2_set, num_segments=args.num_segments,
                                 new_length=data_length2,
                                 modality=args.modality2,
                                 image_tmpl="img_{:05d}.jpg" if args.modality2 in ["RGB", "RGBDiff"] else args.flow_prefix2+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
        ...
        if len(Select_T2_set) > 0:
            dsets['d_pseudo_target_feat2'] = PathFolder(Select_T2_set, num_segments=args.num_segments,
                                 new_length=data_length2,
                                 modality=args.modality2,
                                 image_tmpl="img_{:05d}.jpg" if args.modality2 in ["RGB", "RGBDiff"] else args.flow_prefix2+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
        ...
        if len(Select_T1T2_set) > 0:
            if args.useT1Only:
                dsets['d_pseudo_all2'] = PathFolder(Select_T1_set, num_segments=args.num_segments,
                                 new_length=data_length2,
                                 modality=args.modality2,
                                 image_tmpl="img_{:05d}.jpg" if args.modality2 in ["RGB", "RGBDiff"] else args.flow_prefix2+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
                dsets['d_pseudo_all_feat2'] = PathFolder(Select_T1_set, num_segments=args.num_segments,
                                     new_length=data_length2,
                                     modality=args.modality2,
                                     image_tmpl="img_{:05d}.jpg" if args.modality2 in ["RGB", "RGBDiff"] else args.flow_prefix2+"{}_{:05d}.jpg",
                                     transform=transforms['target'])
            else:
                dsets['d_pseudo_all2'] = PathFolder(Select_T1T2_set, num_segments=args.num_segments,
                                 new_length=data_length2,
                                 modality=args.modality2,
                                 image_tmpl="img_{:05d}.jpg" if args.modality2 in ["RGB", "RGBDiff"] else args.flow_prefix2+"{}_{:05d}.jpg",
                                 transform=transforms['target'])
                dsets['d_pseudo_all_feat2'] = PathFolder(Select_T1T2_set, num_segments=args.num_segments,
                                 new_length=data_length2,
                                 modality=args.modality2,
                                 image_tmpl="img_{:05d}.jpg" if args.modality2 in ["RGB", "RGBDiff"] else args.flow_prefix2+"{}_{:05d}.jpg",
                                 transform=transforms['target'])                
    ...
    # for i, (input, target) in enumerate(source_loader):
        ...
        dom_feat_loss = 0.1*domain_loss_feat
        # + \
        ...
        dom_feat_loss2 = 0.3*domain_loss_feat2
        # + \
        ...
        if i % args.print_freq == 0:
            print(('Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t'
                  ...
                  'Wmain_2: {4:.3f}\t'
                  'l:{l:.3f}'.format(
                   epoch, i, len(source_iter), -0.1, -0.3, batch_time=batch_time,
    ...
    ax.annotate('Main, Main2: %0.4f %0.4f' % \
                (-0.1, -0.1), xy=(1.05, 0.4), xycoords='axes fraction')
