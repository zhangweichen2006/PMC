...
from models_cospcan2 import TSN2
...
def main():
    ...
    if args.dataset == 'ucf101':
        ...
        num_class = 10
    elif args.dataset == 'ucf6':
        num_class = 6
    elif args.dataset == 'olympic6':
        num_class = 6
    elif args.dataset == 'ucf12':
        num_class = 12
    elif args.dataset == 'hmdb12':
        num_class = 12
    else:
    ...
    init_w_main_2 = Variable(torch.FloatTensor([float(args.main_w2)]).cuda(1))
    init_w_l2_2 = Variable(torch.FloatTensor([float(args.form_w2/3)]).cuda(1))
    init_w_l3_2 = Variable(torch.FloatTensor([float(args.form_w2/3)]).cuda(1))
    init_w_l4_2 = Variable(torch.FloatTensor([float(args.form_w2/3)]).cuda(1))
    ...
    model2 = TSN2(num_class, args.num_segments, args.modality2, 
                base_model=args.arch,
    ...
    test_model2 = TSN2(num_class, 1, args.modality2,
                      base_model=args.arch,
                      consensus_type=args.crop_fusion_type,
                      dropout=args.test_dropout)
    ...
    base_policies, source_bottle_policies, sub_bottle_policies, source_cls_policies, \
                                                   domain_cls_policies, disc_w_policies, cd_policies = model.get_optim_policies()


    base_policies2, source_bottle_policies2, sub_bottle_policies2, source_cls_policies2, \
                                                   domain_cls_policies2, disc_w_policies2, _ = model2.get_optim_policies()
    ...
    new_dict_c = copy.deepcopy(cd_policies)
    new_dict_d = copy.deepcopy(cd_policies)
    new_dict_fd = copy.deepcopy(cd_policies)

    new_dict_c[0]['params'] = cd_policies[0]['params'][0]
    new_dict_d[0]['params'] = cd_policies[0]['params'][1]
    new_dict_fd[0]['params'] = cd_policies[0]['params'][2]

    c_policies = new_dict_c
    d_policies = new_dict_d
    fd_policies = new_dict_fd

    optimizer_c = torch.optim.SGD(c_policies,
                                args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)

    optimizer_d = torch.optim.SGD(d_policies,
                                args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)

    optimizer_fd = torch.optim.SGD(fd_policies,
                                args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)
    # if args.evaluate:
    ...
    for epoch in range(args.start_epoch, args.total_epochs):
        ...
        adjust_learning_rate(optimizer_c, 0.1*args.lr, decay)
        adjust_learning_rate(optimizer_d, 0.1*args.lr, decay)
        adjust_learning_rate(optimizer_fd, 0.1*args.lr, decay)
        ...
        prec1, prec1_2, Pseudo_set, Pseudo_set2, Pseudo_set_co, c_1, d_1, fd_1, c_2, d_2, fd_2 = validate(val_iter, val_iter2, model, model2, criterion, domain_criterion, (epoch + 1) * len(dset_loaders['val']), epoch, pre_epochs, optimizer_c, optimizer_d, optimizer_fd)
...
def train(dsets, transforms, dset_loaders, train_source_iter, train_target_iter, train_source_iter2, train_target_iter2, model, model2, Pseudo_set, Pseudo_set2, Pseudo_set_co, criterion, domain_criterion, optimizer, dom_optimizer, dom_w_optimizer, dom_feat_optimizer, optimizer2, dom_optimizer2, dom_w_optimizer2, dom_feat_optimizer2,  epoch, pre_epochs, num_class, c_1, d_1, fd_1, c_2, d_2, fd_2):
    ...
    if epoch >= pre_epochs and epoch % args.skip == 0:
        ...
        Select_confid_set_co = Sorted_confid_set_co[:confid_pseudo_num]
        ...
        Select_confid_set_co2 = Sorted_confid_set_co[:confid_pseudo_num2]
        ...
        # combine from both stream
        Select_T1_set = Select_T1_set + Select_confid_set_co
        Select_T2_set = Select_T2_set + Select_T2_set2
        Select_T1T2_set = Select_T1T2_set + Select_T1T2_set2


        Select_T1_set2 = Select_T1_set2 + Select_confid_set_co2
        Select_T2_set2 = Select_T2_set
        Select_T1T2_set2 = Select_T1T2_set

        print("lenT1", len(Select_T1_set), len(Select_T1_set2), "lenT1T2", len(Select_T1T2_set), len(Select_T1T2_set2))
    ...
    d_target_size = len(dsets['target']) * 2
    d_pseudo_all_size = len(dsets['d_pseudo_all'])

    # pseudo_size = int(round(pseudo_size / 2))
    # d_target_size = int(round(d_target_size / 2))
    # d_pseudo_all_size = int(round(d_pseudo_all_size / 2))
    ...
    d_target_size2 = len(dsets['target2']) * 2
    ...
    # for i, (input, target) in enumerate(source_loader):
        ...
        p_i = epoch / (args.epochs * args.lr_ratio)
        # p = pull_back_to_one(p)

        if (epoch == 0):
            lr_mult = 1 / (1 + np.exp(-3*(source_pointer / len(dsets['source']))))
        else:
            lr_mult = 1

        decay_i = (1. + 10 * p_i)**(-0.75) * lr_mult


        adjust_learning_rate(optimizer, args.lr, decay_i)
        adjust_learning_rate(dom_optimizer, args.lr, decay_i)
        adjust_learning_rate(dom_feat_optimizer, args.lr, decay_i)

        adjust_learning_rate(optimizer2, args.lr2, decay_i)
        adjust_learning_rate(dom_optimizer2, args.lr2, decay_i)
        adjust_learning_rate(dom_feat_optimizer2, args.lr2, decay_i)

        disc_w_decay_i = args.wt * args.wp **p_i
        adjust_learning_rate(dom_w_optimizer, args.lr, disc_w_decay_i)
        adjust_learning_rate(dom_w_optimizer2, args.lr2, disc_w_decay_i)
        ...
        class_loss = c_1.data[0] * compute_new_loss(class_outputs, labels, class_weight)
        ...
        class_loss2 = c_2.data[0] * compute_new_loss(class_outputs2, labels2, class_weight2)
        ...
        domain_outputs = model('dom_train', input=domain_inputs, l=l)
        # , domain_outputs_l2, domain_outputs_l3, \
        #         domain_outputs_l4, , w_l2, w_l3, w_l4 \
        domain_outputs_feat = domain_outputs
        # , domain_outputs_l2_feat, domain_outputs_l3_feat, \
        ...
        dom_loss = d_1.data[0] * abs(args.main_w)*domain_loss
        ...
        dom_feat_loss = fd_1.data[0] * abs(args.main_w)*domain_loss_feat
        # + \
        ...
        domain_outputs2 = model2('dom_train', input=domain_inputs2, l=l)
        # , domain_outputs_l2_2, domain_outputs_l3_2, \
        #         domain_outputs_l4_2, w_main_2, w_l2_2, w_l3_2, w_l4_2 \
                                         
        domain_outputs_feat2 = domain_outputs2
        # , domain_outputs_l2_feat2, domain_outputs_l3_feat2, \
        ...
        dom_loss2 = d_2.data[0] * abs(args.main_w2) * domain_loss_2
        # + \
        ...
        dom_feat_loss2 = fd_2.data[0] * abs(args.main_w2)*domain_loss_feat2
        # + \
        ...
        if i % args.print_freq == 0:
            print(('Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t'
                  ...
                  'Wmain_2: {4:.3f}\t'
                  'l:{l:.3f}'.format(
                   epoch, i, len(source_iter), abs(args.main_w), abs(args.main_w2), batch_time=batch_time,
    ...
    ax.annotate('lr: %0.4f Pre epochs: %d Max epochs: %d' % (args.lr, int(args.pre_ratio * args.total_epochs), args.total_epochs), xy=(1.05, 0.65), xycoords='axes fraction')
    ...
    ax.annotate('Main, Main2: %0.4f %0.4f' % \
                (args.main_w, args.main_w2), xy=(1.05, 0.4), xycoords='axes fraction')
...
def validate(val_iter, val_iter2, model, model2, criterion, domain_criterion, iter, epoch, pre_epochs, optimizer_c, optimizer_d, optimizer_fd, logger=None):
    ...
    for param in model.Modality_Weights.parameters():
    ...
    for cnt in range(len(val_iter)):
        ...
        c_1, d_1, fd_1, c_2, d_2, fd_2 = model('CD_train')
        ...
        # if epoch >= pre_epochs and args.learnCD:
        #   d_loss = 0.1 * torch.abs(  d_2* abs(dom_prob2.data[0] - 0.5) - d_1 * abs(dom_prob.data[0] - 0.5) ) #- 0.01*d_rgb*d_rgb
        #   # print("d_loss", d_loss)
        ...
        #   fd_loss = - 0.1 * torch.abs( fd_2* abs(dom_prob2.data[0] - 0.5) - fd_1 * abs(dom_prob.data[0] - 0.5) ) #- 0.01*d_rgb*d_rgb
        #   # print("d_loss", d_loss)
        ...
        if epoch >= 0  and args.learnCD: #pre_epochs
          print("confid_rate, confid_rate2", confid_rate.data[0], confid_rate2.data[0])
          print("loss, loss2", loss.data[0], loss2.data[0])
          c_loss =  c_1 * confid_rate.data[0] * loss2.data[0] + c_2 * confid_rate2.data[0] * loss.data[0]#+ 0.1*c_rgb*c_rgb
          # print("c_loss", c_loss)
          ...
          c_prob_rgb += confid_rate.data[0]
          c_prob_flow += confid_rate2.data[0] 
    ...
    print('Val Epoch: [{0}]\t'
                  'Time {1} \t'
                  'c_rgb {2:.3f}, d_rgb {3:.3f}, fd_rgb {4:.3f}\t'
                  'Prec@1 {top1.avg:.3f}\t'
                  'Prec2@1 {top1_2.avg:.3f}\t'
                  'Domain {domain_meter.avg:.3f}\t'
                  'Domain2 {domain_meter2.avg:.3f}\t'
                  'c_prob_rgb {c_prob_rgb:.4f}\t'
                  'c_prob_flow {c_prob_flow:.4f}\t'.format(
                   epoch, str(val_time), c_1.data[0], d_1.data[0], fd_1.data[0],
                   top1=top1, top1_2=top1_2, domain_meter=domain_meter, domain_meter2=domain_meter2, c_prob_rgb=c_prob_rgb, c_prob_flow=c_prob_flow)) 

    return top1.avg, top1_2.avg, Pseudo_set, Pseudo_set2, Pseudo_set_co, c_1, d_1, fd_1, c_2, d_2, fd_2
